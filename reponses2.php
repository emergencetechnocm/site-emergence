...
if (isset($_POST['submit'])) {
  ...
  if (!empty($nom) and !empty($email) and !empty($message)) {
    require_once('db.php');
    $req=$pdo->prepare('INSERT INTO reponses2(id_parent,nom,email,messages,datepost) VALUES(?,?,?,?,NOW())');
...
$req=$pdo->prepare('SELECT *FROM reponses2 WHERE id_parent=? ORDER BY id DESC');
        $req->execute(array($_GET['id']));
        while ($reponse=$req->fetch(PDO::FETCH_OBJ)) {
